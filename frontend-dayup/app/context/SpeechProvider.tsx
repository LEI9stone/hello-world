...
import { HEADING_POSITION, SPEECH_RATES } from './speech-types';
...
export function SpeechProvider({
  ...
  // SSR 与客户端首帧都使用 null，避免 hydration 不一致；探测放到 effect 里。
  ...
  const runQueue = useCallback(
    (cursor: number) => {
      ...
      // 每句单独编号，上一句延迟到达的回调也不能改变当前高亮。
      const token = ++tokenRef.current;
      // 在 onstart 前也记录队列位置，避免句间暂停后回到上一句。
  ...
  const speakWord = useCallback(
    (
      ...
      const isHeading =
        ownerSentence === HEADING_POSITION && ownerWord === HEADING_POSITION;
      const cleaned = isHeading ? text.trim() : cleanWord(text) || text.trim();
      if (!cleaned) return;
