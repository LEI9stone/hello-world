import { HEADING_POSITION } from './speech-types';
...
export function chapterText(chapter: Chapter.Data): string {
 ...
 */
export function buildChapterQueue(chapter: Chapter.Data): SpeechStep[] {
...
function spokenText(sentence: Chapter.Sentence): string {
...
function alignWords(
  text: string,
  tokens: readonly { word: Chapter.WordEntry; wordIndex: number }[],
...
/** 词按钮的无障碍标签 */
export function wordAriaLabel(word: Chapter.WordEntry): string {
  if (word.kind === 'link') return `链接 ${word.word}`;
  if (word.kind === 'symbol') return word.word;
  return `${word.word}${word.phonetic ? `，音标 ${word.phonetic}` : ''}，点击发音`;
}
